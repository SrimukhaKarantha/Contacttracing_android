 ...
 * - {@link PurchaseOrderHeadersCreateFragment}: to create or edit a(n) PurchaseOrderHeader,
 * - {@link PurchaseOrderHeadersDetailFragment}: to display the details of a(n) PurchaseOrderHeader,
 * - {@link PurchaseOrderHeadersListFragment}: to list PurchaseOrderHeaders.
 *
 ...
 * what to do.
 */
public class PurchaseOrderHeadersActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<PurchaseOrderHeader> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link PurchaseOrderHeadersListFragment} is
     * getting instantiated (which will load {@link PurchaseOrderHeadersDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new PurchaseOrderHeadersListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable PurchaseOrderHeader entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        PurchaseOrderHeadersCreateFragment fragment = new PurchaseOrderHeadersCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable PurchaseOrderHeader entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new PurchaseOrderHeadersDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new PurchaseOrderHeadersDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable PurchaseOrderHeader entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link PurchaseOrderHeadersCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((PurchaseOrderHeadersActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            PurchaseOrderHeaderViewModel viewModel = new ViewModelProvider(getActivity()).get(PurchaseOrderHeaderViewModel.class);
