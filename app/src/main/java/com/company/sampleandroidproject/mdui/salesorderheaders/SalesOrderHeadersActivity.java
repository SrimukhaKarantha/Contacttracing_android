 ...
 * - {@link SalesOrderHeadersCreateFragment}: to create or edit a(n) SalesOrderHeader,
 * - {@link SalesOrderHeadersDetailFragment}: to display the details of a(n) SalesOrderHeader,
 * - {@link SalesOrderHeadersListFragment}: to list SalesOrderHeaders.
 *
 ...
 * what to do.
 */
public class SalesOrderHeadersActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<SalesOrderHeader> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link SalesOrderHeadersListFragment} is
     * getting instantiated (which will load {@link SalesOrderHeadersDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new SalesOrderHeadersListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable SalesOrderHeader entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        SalesOrderHeadersCreateFragment fragment = new SalesOrderHeadersCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable SalesOrderHeader entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new SalesOrderHeadersDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new SalesOrderHeadersDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable SalesOrderHeader entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link SalesOrderHeadersCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((SalesOrderHeadersActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            SalesOrderHeaderViewModel viewModel = new ViewModelProvider(getActivity()).get(SalesOrderHeaderViewModel.class);
