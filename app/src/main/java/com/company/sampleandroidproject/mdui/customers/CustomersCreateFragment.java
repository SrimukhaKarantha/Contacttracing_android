...
import com.company.sampleandroidproject.databinding.FragmentCustomersCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing Customer entity.
 * This fragment is contained in the {@link CustomersActivity}.
 */
public class CustomersCreateFragment extends InterfacedFragment<Customer> {
    ...
    private Customer customerEntityCopy;

    /** DataBinding generated class */
    private FragmentCustomersCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for Customers to enter values for the properties.
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.customer.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.customer.getLocalName();
        }

        ((CustomersActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            ...
            customerEntity = viewModel.getSelectedEntity().getValue();
        }

        Customer workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (Customer)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            customerEntityCopy = (Customer) customerEntity.copy();
            customerEntityCopy.setEntityTag(customerEntity.getEntityTag());
            customerEntityCopy.setOldEntity(customerEntity);
            customerEntityCopy.setEditLink((customerEntity.getEditLink()));
        } else {
    ...
    private boolean onSaveItem() {
        ...
        ((CustomersActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(customerEntityCopy);
        } else {
            viewModel.update(customerEntityCopy);
        }
        return true;
    }
    ...
    private Customer createCustomer() {
        Customer customerEntity = new Customer(true);
        return customerEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<Customer> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(customerEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        ...
        binding.setCustomer(customerEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isCustomerValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_customer);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.customer.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<Customer> result) {
        String errorMessage;
