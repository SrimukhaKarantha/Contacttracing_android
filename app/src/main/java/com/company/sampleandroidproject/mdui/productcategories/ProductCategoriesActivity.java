 ...
 * - {@link ProductCategoriesCreateFragment}: to create or edit a(n) ProductCategory,
 * - {@link ProductCategoriesDetailFragment}: to display the details of a(n) ProductCategory,
 * - {@link ProductCategoriesListFragment}: to list ProductCategories.
 *
 ...
 * what to do.
 */
public class ProductCategoriesActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<ProductCategory> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link ProductCategoriesListFragment} is
     * getting instantiated (which will load {@link ProductCategoriesDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new ProductCategoriesListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable ProductCategory entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        ProductCategoriesCreateFragment fragment = new ProductCategoriesCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable ProductCategory entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new ProductCategoriesDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new ProductCategoriesDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable ProductCategory entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link ProductCategoriesCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((ProductCategoriesActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            ProductCategoryViewModel viewModel = new ViewModelProvider(getActivity()).get(ProductCategoryViewModel.class);
