package com.company.sampleandroidproject.mdui.salesorderitems;
...
import com.company.sampleandroidproject.viewmodel.salesorderitem.SalesOrderItemViewModel;
 ...
 * - {@link SalesOrderItemsCreateFragment}: to create or edit a(n) SalesOrderItem,
 * - {@link SalesOrderItemsDetailFragment}: to display the details of a(n) SalesOrderItem,
 * - {@link SalesOrderItemsListFragment}: to list SalesOrderItems.
 *
 ...
 * what to do.
 */
public class SalesOrderItemsActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<SalesOrderItem> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link SalesOrderItemsListFragment} is
     * getting instantiated (which will load {@link SalesOrderItemsDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new SalesOrderItemsListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable SalesOrderItem entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        SalesOrderItemsCreateFragment fragment = new SalesOrderItemsCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable SalesOrderItem entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new SalesOrderItemsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new SalesOrderItemsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable SalesOrderItem entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link SalesOrderItemsCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((SalesOrderItemsActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            SalesOrderItemViewModel viewModel = new ViewModelProvider(getActivity()).get(SalesOrderItemViewModel.class);
