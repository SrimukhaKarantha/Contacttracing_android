package com.company.sampleandroidproject.mdui.products;
...
import com.company.sampleandroidproject.viewmodel.product.ProductViewModel;
import com.sap.cloud.android.odata.espmcontainer.Product;

/**
 * This activity handles three kind of {@link InterfacedFragment}s:
 * - {@link ProductsCreateFragment}: to create or edit a(n) Product,
 * - {@link ProductsDetailFragment}: to display the details of a(n) Product,
 * - {@link ProductsListFragment}: to list Products.
 *
 ...
 * what to do.
 */
public class ProductsActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<Product> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link ProductsListFragment} is
     * getting instantiated (which will load {@link ProductsDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new ProductsListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable Product entity) {
        switch (eventId) {
     ...
     */
    private void onItemClicked(@Nullable Product entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new ProductsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new ProductsDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable Product entity) {
        Bundle arguments = new Bundle();
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((ProductsActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            ProductViewModel viewModel = new ViewModelProvider(getActivity()).get(ProductViewModel.class);
