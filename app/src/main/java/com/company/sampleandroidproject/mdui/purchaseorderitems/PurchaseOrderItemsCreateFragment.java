...
import com.company.sampleandroidproject.databinding.FragmentPurchaseorderitemsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing PurchaseOrderItem entity.
 * This fragment is contained in the {@link PurchaseOrderItemsActivity}.
 */
public class PurchaseOrderItemsCreateFragment extends InterfacedFragment<PurchaseOrderItem> {
    ...
    /** PurchaseOrderItem object and it's copy: the modifications are done on the copied object. */
    private PurchaseOrderItem purchaseOrderItemEntity;
    private PurchaseOrderItem purchaseOrderItemEntityCopy;

    /** DataBinding generated class */
    private FragmentPurchaseorderitemsCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for PurchaseOrderItems to enter values for the properties.
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.purchaseOrderItem.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.purchaseOrderItem.getLocalName();
        }

        ((PurchaseOrderItemsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            purchaseOrderItemEntity = createPurchaseOrderItem();
        } else {
            purchaseOrderItemEntity = viewModel.getSelectedEntity().getValue();
        }

        PurchaseOrderItem workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (PurchaseOrderItem)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            purchaseOrderItemEntityCopy = (PurchaseOrderItem) purchaseOrderItemEntity.copy();
            purchaseOrderItemEntityCopy.setEntityTag(purchaseOrderItemEntity.getEntityTag());
            purchaseOrderItemEntityCopy.setOldEntity(purchaseOrderItemEntity);
            purchaseOrderItemEntityCopy.setEditLink((purchaseOrderItemEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            purchaseOrderItemEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, purchaseOrderItemEntityCopy);
    ...
    private boolean onSaveItem() {
        ...
        ((PurchaseOrderItemsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(purchaseOrderItemEntityCopy);
        } else {
            viewModel.update(purchaseOrderItemEntityCopy);
        }
        return true;
    }
    ...
    private PurchaseOrderItem createPurchaseOrderItem() {
        PurchaseOrderItem purchaseOrderItemEntity = new PurchaseOrderItem(true);
        return purchaseOrderItemEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<PurchaseOrderItem> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(purchaseOrderItemEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentPurchaseorderitemsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setPurchaseOrderItem(purchaseOrderItemEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isPurchaseOrderItemValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_purchaseorderitem);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.purchaseOrderItem.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<PurchaseOrderItem> result) {
        String errorMessage;
