package com.company.sampleandroidproject.mdui.suppliers;
...
import com.company.sampleandroidproject.viewmodel.supplier.SupplierViewModel;
...
public class SuppliersListFragment extends InterfacedFragment<Supplier> {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        activityTitle = getString(EntitySetListActivity.EntitySetName.Suppliers.getTitleId());
    ...
    public void onViewStateRestored(@Nullable Bundle savedInstanceState) {
        ...
        this.adapter = new SupplierListAdapter(currentActivity);
    ...
    private void prepareViewModel() {
        if( navigationPropertyName != null && parentEntityData != null ) {
            viewModel = new ViewModelProvider(currentActivity, new EntityViewModelFactory(currentActivity.getApplication(), navigationPropertyName, parentEntityData))
                .get(SupplierViewModel.class);
        } else {
            viewModel = new ViewModelProvider(currentActivity).get(SupplierViewModel.class);
            viewModel.initialRead(this::showError);
        }

        viewModel.getObservableItems().observe(getViewLifecycleOwner(), suppliers -> {
            if (suppliers != null) {
                adapter.setItems(suppliers);

                Supplier item = containsItem(suppliers, viewModel.getSelectedEntity().getValue());
                if (item == null) {
                    item = suppliers.isEmpty() ? null : suppliers.get(0);
                }

                if (item == null) {
                    hideDetailFragment();
                } else {
                    viewModel.setInFocusId(adapter.getItemIdForSupplier(item));
                    if (currentActivity.getResources().getBoolean(R.bool.two_pane)) {
                        viewModel.setSelectedEntity(item);
                        if (!isInActionMode && !((SuppliersActivity) currentActivity).isNavigationDisabled) {
    ...
    private Supplier containsItem(List<Supplier> items, Supplier item) {
        Supplier found = null;
        if( item != null ) {
            for( Supplier entity: items ) {
                if( adapter.getItemIdForSupplier(entity) == adapter.getItemIdForSupplier(item)) {
                    found = entity;
                    break;
                }
            }
    ...
    private Supplier setItemIdSelected(int itemId) {
        LiveData<List<Supplier>> liveData = viewModel.getObservableItems();
        List<Supplier> suppliers = liveData.getValue();
        if (suppliers != null && suppliers.size() > 0) {
            viewModel.setInFocusId(adapter.getItemIdForSupplier(suppliers.get(itemId)));
            return suppliers.get(itemId);
        }
        return null;
    }
     ...
     */
    public class SuppliersListActionMode implements ActionMode.Callback {
        @Override
        ...
        public boolean onActionItemClicked(ActionMode actionMode, MenuItem menuItem) {
            switch (menuItem.getItemId()) {
                case R.id.update_item:
                    Supplier supplierEntity = viewModel.getSelected(0);
    ...
    public class SupplierListAdapter extends RecyclerView.Adapter<SupplierListAdapter.ViewHolder> {
         ...
         */
        @WorkerThread
        public void setItems(@NonNull List<Supplier> currentSuppliers) {
            if (suppliers == null) {
                suppliers = new ArrayList<>(currentSuppliers);
                ...
                DiffUtil.DiffResult result = DiffUtil.calculateDiff(new DiffUtil.Callback() {
                    ...
                    public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                        return suppliers.get(oldItemPosition).getEntityKey().toString().equals(
                                currentSuppliers.get(newItemPosition).getEntityKey().toString());
                    }

                    @Override
                    public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
                        Supplier supplierEntity = suppliers.get(oldItemPosition);
                        return !supplierEntity.isUpdated() && currentSuppliers.get(newItemPosition).equals(supplierEntity);
                    }
        ...
        public final long getItemId(int position) {
            return getItemIdForSupplier(suppliers.get(position));
        }
        ...
        public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
            ...
            final Supplier supplierEntity = suppliers.get(holder.getAdapterPosition());
            ...
            boolean isActive = getItemIdForSupplier(supplierEntity) == viewModel.getInFocusId();
            if (isActive) {
                setItemIdSelected(holder.getAdapterPosition());
            }
            boolean isSupplierSelected = viewModel.selectedContains(supplierEntity);
            ...
            holder.setSelected(isSupplierSelected);
        ...
        private void processClickAction(@NonNull ViewHolder viewHolder, @NonNull Supplier supplierEntity) {
            resetPreviouslyClicked();
            setViewBackground(viewHolder.objectCell, false, true);
            viewModel.setInFocusId(getItemIdForSupplier(supplierEntity));
        }
         ...
         */
        private void setOnClickListener(@NonNull ViewHolder holder, @NonNull Supplier supplierEntity) {
            holder.view.setOnClickListener(view -> {
                boolean isNavigationDisabled = ((SuppliersActivity) currentActivity).isNavigationDisabled;
        ...
        public class onActionModeStartClickListener implements View.OnClickListener, View.OnLongClickListener {
            ...
            private boolean onAnyKindOfClick() {
                ...
                if( isNavigationDisabled ) {
                    ...
                    if (!isInActionMode) {
                        actionMode = ((AppCompatActivity) currentActivity).startSupportActionMode(new SuppliersListActionMode());
                        adapter.notifyDataSetChanged();
                    }
        ...
        private void setDetailImage(@NonNull ViewHolder viewHolder, @NonNull Supplier supplierEntity) {
            if (isInActionMode) {
                int drawable;
         ...
         * @param supplierEntity associated with this ViewHolder
         */
        private void setOnCheckedChangeListener(@NonNull ViewHolder holder, @NonNull Supplier supplierEntity) {
        ...
        private void populateObjectCell(@NonNull ViewHolder viewHolder, @NonNull Supplier supplierEntity) {
         ...
         */
        private void setViewBackground(@NonNull View view, boolean isSupplierSelected, boolean isActive) {
         ...
         */
        private long getItemIdForSupplier(Supplier supplierEntity) {
            return supplierEntity.getEntityKey().toString().hashCode();
        }
