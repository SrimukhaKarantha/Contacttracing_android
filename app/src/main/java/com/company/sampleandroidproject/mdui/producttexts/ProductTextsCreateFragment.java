...
import com.company.sampleandroidproject.databinding.FragmentProducttextsCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing ProductText entity.
 * This fragment is contained in the {@link ProductTextsActivity}.
 */
public class ProductTextsCreateFragment extends InterfacedFragment<ProductText> {
    ...
    private ProductText productTextEntity;
    private ProductText productTextEntityCopy;

    /** DataBinding generated class */
    private FragmentProducttextsCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** ProductText ViewModel */
    ...
    /**
     * This fragment is used for both update and create for ProductTexts to enter values for the properties.
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.productText.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.productText.getLocalName();
        }

        ((ProductTextsActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            productTextEntity = createProductText();
        } else {
            productTextEntity = viewModel.getSelectedEntity().getValue();
        }

        ProductText workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (ProductText)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            productTextEntityCopy = (ProductText) productTextEntity.copy();
            productTextEntityCopy.setEntityTag(productTextEntity.getEntityTag());
            productTextEntityCopy.setOldEntity(productTextEntity);
            productTextEntityCopy.setEditLink((productTextEntity.getEditLink()));
        } else {
    ...
    private boolean onSaveItem() {
        ...
        ((ProductTextsActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(productTextEntityCopy);
        } else {
            viewModel.update(productTextEntityCopy);
        }
        return true;
    }
     ...
     */
    private ProductText createProductText() {
        ProductText productTextEntity = new ProductText(true);
        return productTextEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<ProductText> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(productTextEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentProducttextsCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setProductText(productTextEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isProductTextValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_producttext);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.productText.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<ProductText> result) {
        String errorMessage;
