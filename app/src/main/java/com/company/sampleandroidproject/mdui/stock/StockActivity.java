package com.company.sampleandroidproject.mdui.stock;
...
import com.company.sampleandroidproject.viewmodel.stock.StockViewModel;
import com.sap.cloud.android.odata.espmcontainer.Stock;

/**
 * This activity handles three kind of {@link InterfacedFragment}s:
 * - {@link StockCreateFragment}: to create or edit a(n) Stock,
 * - {@link StockDetailFragment}: to display the details of a(n) Stock,
 * - {@link StockListFragment}: to list Stock.
 *
 ...
 * what to do.
 */
public class StockActivity extends AppCompatActivity implements InterfacedFragment.InterfacedFragmentListener<Stock> {
    ...
    /**
     * In this lifecycle state, some instance state variables gets loaded, {@link StockListFragment} is
     * getting instantiated (which will load {@link StockDetailFragment} if needed), visibilities of
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            ...
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new StockListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit();
        }
    ...
    */
    ...
    @Override
    public void onFragmentStateChange(int eventId, @Nullable Stock entity) {
        switch (eventId) {
    ...
    private void onCreateNewItem() {
        ...
        StockCreateFragment fragment = new StockCreateFragment();
     ...
     */
    private void onItemClicked(@Nullable Stock entity) {
        if( !isMasterDetailView ) {
            getSupportFragmentManager().beginTransaction()
                .replace(R.id.masterFrame, new StockDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if( detail == null && entity != null ) {
                getSupportFragmentManager().beginTransaction()
                    .replace(R.id.detailFrame, new StockDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                    .commit();
                return;
            }
     ...
     */
    private void onEditItem(@Nullable Stock entity) {
        Bundle arguments = new Bundle();
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link StockCreateFragment}.
     */
    ...
    public static class ConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            ...
            builder.setPositiveButton(R.string.before_navigation_dialog_positive_button, (dialog, which) -> {
                ((StockActivity)getActivity()).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null);
            });
    ...
    public static class DeleteConfirmationDialogFragment extends DialogFragment {
        ...
        public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
            AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(getActivity(), R.style.AlertDialogStyle));
            StockViewModel viewModel = new ViewModelProvider(getActivity()).get(StockViewModel.class);
