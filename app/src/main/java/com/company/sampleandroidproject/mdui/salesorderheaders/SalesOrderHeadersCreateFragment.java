...
import com.company.sampleandroidproject.databinding.FragmentSalesorderheadersCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing SalesOrderHeader entity.
 * This fragment is contained in the {@link SalesOrderHeadersActivity}.
 */
public class SalesOrderHeadersCreateFragment extends InterfacedFragment<SalesOrderHeader> {
    ...
    private SalesOrderHeader salesOrderHeaderEntity;
    private SalesOrderHeader salesOrderHeaderEntityCopy;

    /** DataBinding generated class */
    private FragmentSalesorderheadersCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for SalesOrderHeaders to enter values for the properties.
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.salesOrderHeader.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.salesOrderHeader.getLocalName();
        }

        ((SalesOrderHeadersActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            salesOrderHeaderEntity = createSalesOrderHeader();
        } else {
            salesOrderHeaderEntity = viewModel.getSelectedEntity().getValue();
        }

        SalesOrderHeader workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (SalesOrderHeader)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            salesOrderHeaderEntityCopy = (SalesOrderHeader) salesOrderHeaderEntity.copy();
            salesOrderHeaderEntityCopy.setEntityTag(salesOrderHeaderEntity.getEntityTag());
            salesOrderHeaderEntityCopy.setOldEntity(salesOrderHeaderEntity);
            salesOrderHeaderEntityCopy.setEditLink((salesOrderHeaderEntity.getEditLink()));
        } else {
    ...
    private boolean onSaveItem() {
        ...
        ((SalesOrderHeadersActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(salesOrderHeaderEntityCopy);
        } else {
            viewModel.update(salesOrderHeaderEntityCopy);
        }
        return true;
    }
     ...
     */
    private SalesOrderHeader createSalesOrderHeader() {
        SalesOrderHeader salesOrderHeaderEntity = new SalesOrderHeader(true);
        return salesOrderHeaderEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<SalesOrderHeader> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(salesOrderHeaderEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentSalesorderheadersCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setSalesOrderHeader(salesOrderHeaderEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isSalesOrderHeaderValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_salesorderheader);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.salesOrderHeader.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<SalesOrderHeader> result) {
        String errorMessage;
