package com.company.sampleandroidproject.mdui.customers;
...
import com.company.sampleandroidproject.viewmodel.customer.CustomerViewModel;
...
public class CustomersListFragment extends InterfacedFragment<Customer> {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        activityTitle = getString(EntitySetListActivity.EntitySetName.Customers.getTitleId());
    ...
    public void onViewStateRestored(@Nullable Bundle savedInstanceState) {
        ...
        this.adapter = new CustomerListAdapter(currentActivity);
    ...
    private void prepareViewModel() {
        ...
        viewModel.getObservableItems().observe(getViewLifecycleOwner(), customers -> {
            if (customers != null) {
                adapter.setItems(customers);

                Customer item = containsItem(customers, viewModel.getSelectedEntity().getValue());
                if (item == null) {
                    item = customers.isEmpty() ? null : customers.get(0);
                }

                if (item == null) {
                    hideDetailFragment();
                } else {
                    viewModel.setInFocusId(adapter.getItemIdForCustomer(item));
                    if (currentActivity.getResources().getBoolean(R.bool.two_pane)) {
                        viewModel.setSelectedEntity(item);
                        if (!isInActionMode && !((CustomersActivity) currentActivity).isNavigationDisabled) {
    ...
    private Customer containsItem(List<Customer> items, Customer item) {
        Customer found = null;
        if( item != null ) {
            for( Customer entity: items ) {
                if( adapter.getItemIdForCustomer(entity) == adapter.getItemIdForCustomer(item)) {
                    found = entity;
                    break;
                }
            }
    ...
    private Customer setItemIdSelected(int itemId) {
        LiveData<List<Customer>> liveData = viewModel.getObservableItems();
        List<Customer> customers = liveData.getValue();
        if (customers != null && customers.size() > 0) {
            viewModel.setInFocusId(adapter.getItemIdForCustomer(customers.get(itemId)));
            return customers.get(itemId);
        }
        return null;
    }
     ...
     */
    public class CustomersListActionMode implements ActionMode.Callback {
        @Override
        ...
        public boolean onActionItemClicked(ActionMode actionMode, MenuItem menuItem) {
            switch (menuItem.getItemId()) {
                case R.id.update_item:
                    Customer customerEntity = viewModel.getSelected(0);
    ...
    public class CustomerListAdapter extends RecyclerView.Adapter<CustomerListAdapter.ViewHolder> {
         ...
         */
        @WorkerThread
        public void setItems(@NonNull List<Customer> currentCustomers) {
            if (customers == null) {
                customers = new ArrayList<>(currentCustomers);
                ...
                DiffUtil.DiffResult result = DiffUtil.calculateDiff(new DiffUtil.Callback() {
                    ...
                    public boolean areItemsTheSame(int oldItemPosition, int newItemPosition) {
                        return customers.get(oldItemPosition).getEntityKey().toString().equals(
                                currentCustomers.get(newItemPosition).getEntityKey().toString());
                    }

                    @Override
                    public boolean areContentsTheSame(int oldItemPosition, int newItemPosition) {
                        Customer customerEntity = customers.get(oldItemPosition);
                        return !customerEntity.isUpdated() && currentCustomers.get(newItemPosition).equals(customerEntity);
                    }
        ...
        public final long getItemId(int position) {
            return getItemIdForCustomer(customers.get(position));
        }
        ...
        public void onBindViewHolder(@NonNull final ViewHolder holder, final int position) {
            ...
            final Customer customerEntity = customers.get(holder.getAdapterPosition());
            ...
            boolean isActive = getItemIdForCustomer(customerEntity) == viewModel.getInFocusId();
            if (isActive) {
                setItemIdSelected(holder.getAdapterPosition());
            }
            boolean isCustomerSelected = viewModel.selectedContains(customerEntity);
            ...
            holder.setSelected(isCustomerSelected);
        ...
        private void processClickAction(@NonNull ViewHolder viewHolder, @NonNull Customer customerEntity) {
            resetPreviouslyClicked();
            setViewBackground(viewHolder.objectCell, false, true);
            viewModel.setInFocusId(getItemIdForCustomer(customerEntity));
        }
         ...
         */
        private void setOnClickListener(@NonNull ViewHolder holder, @NonNull Customer customerEntity) {
            holder.view.setOnClickListener(view -> {
                boolean isNavigationDisabled = ((CustomersActivity) currentActivity).isNavigationDisabled;
        ...
        public class onActionModeStartClickListener implements View.OnClickListener, View.OnLongClickListener {
            ...
            private boolean onAnyKindOfClick() {
                ...
                if( isNavigationDisabled ) {
                    ...
                    if (!isInActionMode) {
                        actionMode = ((AppCompatActivity) currentActivity).startSupportActionMode(new CustomersListActionMode());
                        adapter.notifyDataSetChanged();
                    }
        ...
        private void setDetailImage(@NonNull ViewHolder viewHolder, @NonNull Customer customerEntity) {
            if (isInActionMode) {
                int drawable;
         ...
         * @param customerEntity associated with this ViewHolder
         */
        private void setOnCheckedChangeListener(@NonNull ViewHolder holder, @NonNull Customer customerEntity) {
        ...
        private void populateObjectCell(@NonNull ViewHolder viewHolder, @NonNull Customer customerEntity) {
         ...
         */
        private void setViewBackground(@NonNull View view, boolean isCustomerSelected, boolean isActive) {
         ...
         */
        private long getItemIdForCustomer(Customer customerEntity) {
            return customerEntity.getEntityKey().toString().hashCode();
        }
