package com.company.sampleandroidproject.mdui.productcategories;
...
import com.company.sampleandroidproject.databinding.FragmentProductcategoriesCreateBinding;
...
import com.company.sampleandroidproject.viewmodel.productcategory.ProductCategoryViewModel;
...
/**
 * A fragment that presents a screen to either create or update an existing ProductCategory entity.
 * This fragment is contained in the {@link ProductCategoriesActivity}.
 */
public class ProductCategoriesCreateFragment extends InterfacedFragment<ProductCategory> {
    ...
    private ProductCategory productCategoryEntityCopy;

    /** DataBinding generated class */
    private FragmentProductcategoriesCreateBinding binding;
    ...
    /**
     * This fragment is used for both update and create for ProductCategories to enter values for the properties.
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.productCategory.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.productCategory.getLocalName();
        }

        ((ProductCategoriesActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            productCategoryEntity = createProductCategory();
        } else {
            productCategoryEntity = viewModel.getSelectedEntity().getValue();
        }

        ProductCategory workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (ProductCategory)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            productCategoryEntityCopy = (ProductCategory) productCategoryEntity.copy();
            productCategoryEntityCopy.setEntityTag(productCategoryEntity.getEntityTag());
            productCategoryEntityCopy.setOldEntity(productCategoryEntity);
            productCategoryEntityCopy.setEditLink((productCategoryEntity.getEditLink()));
        } else {
    ...
    private boolean onSaveItem() {
        ...
        ((ProductCategoriesActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(productCategoryEntityCopy);
        } else {
            viewModel.update(productCategoryEntityCopy);
        }
        return true;
    }
    ...
    private ProductCategory createProductCategory() {
        ProductCategory productCategoryEntity = new ProductCategory(true);
        return productCategoryEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<ProductCategory> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(productCategoryEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentProductcategoriesCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setProductCategory(productCategoryEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isProductCategoryValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_productcategory);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.productCategory.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<ProductCategory> result) {
        String errorMessage;
