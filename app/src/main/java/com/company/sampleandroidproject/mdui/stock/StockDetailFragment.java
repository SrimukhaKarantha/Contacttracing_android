...
import com.company.sampleandroidproject.databinding.FragmentStockDetailBinding;
 ...
 */
public class StockDetailFragment extends InterfacedFragment<Stock> {

    /** Generated data binding class based on layout file */
    private FragmentStockDetailBinding binding;

    /** Stock entity to be displayed */
    private Stock stockEntity = null;
     ...
     */
    private void setDetailImage(@NonNull ObjectHeader objectHeader, @NonNull Stock stockEntity) {
        if (stockEntity.getDataValue(Stock.lotSize) != null && !stockEntity.getDataValue(Stock.lotSize).toString().isEmpty()) {
            objectHeader.setDetailImageCharacter(stockEntity.getDataValue(Stock.lotSize).toString().substring(0, 1));
        } else {
    ...
    private void setupObjectHeader() {
        ...
        if (secondToolbar != null) {
            secondToolbar.setTitle(stockEntity.getEntityType().getLocalName());
        } else {
            currentActivity.setTitle(stockEntity.getEntityType().getLocalName());
        }
        ...
        if (objectHeader != null) {
            ...
            objectHeader.setSubheadline(EntityKeyUtil.getOptionalEntityKey(stockEntity));
            ...
            setDetailImage(objectHeader, stockEntity);
