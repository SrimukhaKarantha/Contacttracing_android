package com.company.sampleandroidproject.repository;
...
import com.company.sampleandroidproject.archcomp.SingleLiveEvent;
import com.company.sampleandroidproject.mediaresource.EntityMediaResource;

import com.company.sampleandroidproject.repository.OperationResult.Operation;
...
public class Repository<T extends EntityValue> {
     ...
     */
    public Repository(@NonNull ESPMContainer eSPMContainer, @NonNull EntitySet entitySet, @Nullable Property orderByProperty) {
        this.eSPMContainer = eSPMContainer;
        this.entitySet = entitySet;
        this.orderByProperty = orderByProperty;
        observableEntities = new MutableLiveData<>();
        if (EntityMediaResource.isV4(eSPMContainer.getMetadata().getVersionCode()) && EntityMediaResource.hasMediaResources(entitySet)) {
    ...
    public void read(EntityValue parent, String navigationPropertyName) {
        ...
        eSPMContainer.loadPropertyAsync(navigationProperty, parent, dataQuery,
            () -> {
    ...
    public void create(@NonNull T newEntity, @NonNull StreamBase media) {
        if (newEntity.getEntityType().isMedia()) {
            eSPMContainer.createMediaAsync(newEntity, media,
                () -> {
    ...
    public void create(@NonNull T newEntity) {
        ...
        eSPMContainer.createEntityAsync(newEntity,
            () -> {
                insertToCache(newEntity, entities);
                if (relatedEntities.size() > 0) {
                    insertToCache(newEntity, relatedEntities);
    ...
    public void update(@NonNull T updateEntity) {
        eSPMContainer.updateEntityAsync(updateEntity,
            () -> {
    ...
    public void delete(@NonNull List<T> deleteEntities) {
        ...
        eSPMContainer.applyChangesAsync(deleteChangeSet,
            () -> {
